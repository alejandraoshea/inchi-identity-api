...
class UnificationChange:
    ...
    structure_type: str
    smiles_to_inchi: Optional[str]
    normalized_inchi: Optional[str]
    canonical_inchi: str
...
class SimpleMgfDeduplicator:
    ...
    def get_canonical_form_with_steps(self, structure: str, structure_type: str) -> dict:
        steps = {
            "smiles_to_inchi": None,
        ...
        if structure_type == "SMILES":
            ...
            if mol:
                ...
                working = steps["smiles_to_inchi"]
            else:
                steps["canonical_inchi"] = structure
                return steps
        else:
            working = structure

        canonical = self.get_canonical_form(working)
        steps["normalized_inchi"] = canonical if canonical != working else None
        steps["canonical_inchi"] = canonical
        return steps

    def _append_change(self, structure: str, field_name: str, canonical_inchi: str):
        steps = self.get_canonical_form_with_steps(structure, field_name)
        self.changes_log.append(UnificationChange(
            original_structure=structure,
            structure_type=field_name,
            smiles_to_inchi=steps["smiles_to_inchi"],
            normalized_inchi=steps["normalized_inchi"],
    ...
    def unify_inchis_in_file(self, entries: List[Dict], source_file: str) -> List[Dict]:
        ...
        for entry in entries:
            ...
            if found_canonical:
                if structure != found_canonical:
                    self._append_change(structure, field_name, found_canonical)
                ...
                if structure != canonical_struct:
                    self._append_change(structure, field_name, canonical_struct)
        ...
        for entry in entries:
            ...
            if structure and structure in canonical_map:
                canonical_struct, _ = canonical_map[structure]
            ...
            if structure_b:
                ...
                if matched_canonical:
                    if structure_b != matched_canonical:
                        self._append_change(structure_b, field_b, matched_canonical)
    ...
    def process_files(
        ...
        changes_before = len(self.changes_log)
        all_entries = self.cross_unify(entries_a_unified, entries_b_unified, source_a, source_b)
